...
ggplot(data=boro_law, aes(x=Year, y=count, fill=LAW_CAT_CD)) + geom_bar(stat="identity") + facet_grid(.~BORO_NM) + geom_text(aes(label=Prct), position = position_stack(vjust = 0.5), size = 2) + theme(axis.text.x = element_text(angle=90, hjust=1))  #facet_grid(cols=vars(drv))
...
ggplot(data = data_m_highlow, aes(x=RIDERSHIP, y=total)) + geom_point()

ggplot(data = data_m_highlow, aes(x=RIDERSHIP, y=Prct)) + geom_point()
...
ggplot(data = incidenttime_all, aes(x=Hour, y=Total, group=Year, color=Year)) + geom_line()
...
ggplot(data = ridership_incident1, aes(x = Week, Subways..Total.Estimated.Ridership, y = Subways..Total.Estimated.Ridership)) + geom_boxplot()
```
...
ggplot(data = ridership_incident2, aes(x = Week, Subways..Total.Estimated.Ridership, y = Subways..Total.Estimated.Ridership)) + geom_boxplot()
