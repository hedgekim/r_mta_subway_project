...
ggplot(data = df_boro, aes(x=Year, y=incident_prcnt, group=BORO_NM, color=BORO_NM)) + geom_line() + ylim(0,0.0025)
...
ggplot(data=law_cat_year, aes(x=Year, y=count, fill=LAW_CAT_CD)) + geom_bar(stat="identity") + geom_text(aes(label=Prct), position = position_stack(vjust = 0.5), size = 3)
...
ggplot(data=boro_law, aes(x=Year, y=count, fill=LAW_CAT_CD)) + geom_bar(stat="identity") + facet_grid(.~BORO_NM) + geom_text(aes(label=Prct), position = position_stack(vjust = 0.5), size = 2) #facet_grid(cols=vars(drv))
...
data_lat_long = data %>%
  ...
  select(Latitude, Longitude, Lat_Lon, TRANSIT_DISTRICT)
data_lat_long

left_join(top_manhattan_station, distinct(data_lat_long, Lat_Lon), by="Lat_Lon")
...
data_m_top = data %>% #times square/port authority
  mutate(STATION_NAME = recode(STATION_NAME, "42 ST.-PORT AUTHORITY BUS TERM" = "42 ST.-TIMES SQUARE/PORT AUTHORITY", "42 ST.-TIMES SQUARE" = "42 ST.-TIMES SQUARE/PORT AUTHORITY")) %>%
  mutate(Lat_Lon = ifelse(STATION_NAME == "42 ST.-TIMES SQUARE/PORT AUTHORITY", "(40.75604353500006, -73.98694954399997)", Lat_Lon)) %>% #group grand central coordinates
  mutate(Lat_Lon = ifelse(STATION_NAME == "42 ST.-GRAND CENTRAL", "(40.751449824000076, -73.97605054499998)", Lat_Lon)) %>% #group Herald Square Coordinates
  mutate(Lat_Lon = ifelse(STATION_NAME == "34 ST.-HERALD SQ.", "(40.749796581000055, -73.98777004399994)", Lat_Lon))
